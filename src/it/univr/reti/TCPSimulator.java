...
public class TCPSimulator {
    ...
    public TCPSimulator(int mssBytes, int dataBytes, SSTRESHValue sstresh, double[][] networkDowns, double[][] rcvwnds, double rtt, RTOValue rto) {
    	if(rcvwnds.length < 1) throw new IllegalArgumentException("Invalid length of rcvwnds (must be > 0)");
    ...
    public void simulate() {
        ...
        while(data >= 0){
        	if(!isNetworkDown()) {
                ...
                if(cwnd < ssthresh) cwnd = Math.min(Math.min(cwnd + sent, ssthresh), nextRcvwnd);	//calculation of cwnd for the next iteration
                else 				cwnd = Math.min(cwnd + sent / cwnd, nextRcvwnd);				//based on ssthresh
                ...
                System.out.println("[!]\t---> Segments sent at [" + new DecimalFormat("##.#").format(time * rtt) + "] were lost, restoring cwnd " + (rtoScaleFactor > 1 ? ", doubling and " : "and ") + "waiting RTO until [" + new DecimalFormat("##.#").format(time * rtt + rto * rtoScaleFactor) + "]... (" + rtoScaleFactor + "x base RTO)");
                ...
                if(rtoScaleFactor == 8) {					//rto doubling limit check
                	System.out.println("[!]\t---> Reached maximum RTO (4x base RTO) and timed out. Connection closed at [" + time + "]");
                	return;
                }
            }
        }
    ...
    private boolean isNetworkDown() {
        for(int i = 0; i < networkDowns.length; i++){
            ...
            if(time * rtt >= networkDowns[i][0] && time * rtt < networkDowns[i][1] || Math.abs(time * rtt - networkDowns[i][0]) < rtt) 
                return true;
        }
    ...
    private int getNextRcvwnd() {
    	for(int i = time; i > 0; i--) {
    		...
    		if(i < rcvwnds.length && rcvwnds[i] != 0) return rcvwnds[i] / mssBytes;
    	}
